...
class GF_Partial_Entries extends GFFeedAddOn {
	...
	function maybe_save_partial_entry( $form_id ) {
		...
		global $wpdb;
		//$wpdb->insert('zxf_test',array("content"=>"shear query:".'select id from wp_gf_entry where form_id='.$form['fields'][0][formId]." and ip='".$_SERVER['REMOTE_ADDR']."' and  ADDTIME(NOW(),'7:00:00') <  ADDTIME(date_created, '0:2:00') " ));   

        $wpdb->get_results('select id from wp_gf_entry where form_id='.$form['fields'][0][formId]." and ip='".$_SERVER['REMOTE_ADDR']."' and  ADDTIME(NOW(),'7:00:00') <  ADDTIME(date_created, '1:2:00') " );
          if($wpdb->num_rows){return false;}
		 		//$wpdb->insert('zxf_test',array("content"=>"shear formid:".$form['fields'][0][formId]));   
